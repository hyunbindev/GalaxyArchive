...
internal class ArticleViewCountRedisAdapter(
    ...
    override fun getViewCountDeltas(articleIds: List<Long>): Map<Long, Long> {

        val keys = articleIds.map { viewCountDeltaKey(it) }
        ...
        return articleIds.zip(values)
            .associate { (articleId, value)->articleId to (value?.toLongOrNull() ?: 0) }
    }
    ...
    private fun recordAnonymousView(
        ...
        val visitorKey = visitorViewKey(articleId, visitorId)
    ...
    private fun recordUserView(
        ...
        val visitorKey = visitorViewKey(articleId, visitorId)
        val userKey = userViewKey(articleId, userId)

        val isVisitorViewed = redisTemplate.hasKey(visitorKey) ?: false
        val isUserViewed = redisTemplate.hasKey(userKey) ?: false

        if (isVisitorViewed || isUserViewed) {
            // 다른 브라우저에서 동일 user로 접근
            if (!isVisitorViewed) {
                redisTemplate.opsForValue()
                    .set(visitorKey, "1", VIEW_RECORD_TTL)
            }

            // 비로그인으로 본 뒤 로그인
            if (!isUserViewed) {
                redisTemplate.opsForValue()
                    .set(userKey, "1", VIEW_RECORD_TTL)
            }
    ...
    private fun incrementViewCountDelta(articleId: Long) {
        redisTemplate.opsForValue()
            .increment(viewCountDeltaKey(articleId))
    }
    ...
    private fun viewCountDeltaKey(articleId: Long): String =
